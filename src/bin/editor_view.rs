...
use eframe::egui::{
    self, Align2, Color32, Rounding, Id, Pos2, Rect, Sense, Stroke, TextFormat, TextStyle, pos2, vec2,
};
...
if ed.mode == Mode::Edit {
    ...
    painter.line_segment(
        [pos2(content_left - 4.0, text_rect.top()), pos2(content_left - 4.0, text_rect.bottom())],
    ...
    for row in 0..rows {
        ...
        if y >= b.lines.len() {
            painter.text(pos2(text_rect.left() + 4.0, y_px), Align2::LEFT_TOP,
                "~", font_id.clone(), p.text_dim);
            continue;
        }
        ...
        if y == b.cy && sel.is_none() {
            painter.rect_filled(
                Rect::from_min_max(pos2(text_rect.left(), y_px),
                                   pos2(text_rect.right(), y_px + row_h)),
                Rounding::same(2.0), p.curline,
            );
        }
        ...
        gj.append(&num, 0.0, TextFormat { font_id: font_id.clone(), color: gcol, ..Default::default() });
        ...
        for (ci, ch) in line.chars().enumerate() {
            ...
            if vcol + cw_ch <= col_off { vcol += cw_ch; continue; }
            if vcol >= col_off + text_w_chars { break; }
            if first_vcol.is_none() { first_vcol = Some(vcol); }
            let fg = tok_color(toks.get(ci).copied().unwrap_or(Tok::Normal), p);
            let bg = if in_sel(ci) { p.selection } else { Color32::TRANSPARENT };
            let s: String = if ch == '\t' { " ".repeat(cw_ch) } else { ch.to_string() };
            cj.append(&s, 0.0, TextFormat { font_id: font_id.clone(), color: fg, background: bg, ..Default::default() });
            vcol += cw_ch;
        }
        let galley = fonts.layout_job(cj);
        let x = content_left + (first_vcol.unwrap_or(0) as f32 - col_off as f32) * cw;
        let clip = Rect::from_min_max(pos2(content_left, text_rect.top()),
