...
use eframe::egui::{self, Align2, Color32, FontId, RichText, Rounding, Sense, Vec2, pos2};
...
impl FileTree {
    ...
    pub fn show(&mut self, ui: &mut egui::Ui, ed: &mut Editor, p: &Palette, media_slot: &mut Option<PathBuf>) {
        ...
        ui.horizontal(|ui| {
            ui.label(RichText::new("EXPLORER").size(10.5).color(p.text_dim).strong());
        ...
        let root_name = self.root_path.file_name()
            .map(|n| n.to_string_lossy().to_string())
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| self.root_path.display().to_string());
        ui.add_space(4.0);
        ui.horizontal(|ui| {
            ui.label(RichText::new(format!("🗀  {}", root_name)).size(12.5).color(p.fg).strong());
        }).response
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            show_nodes(ui, &self.root_nodes, ed, p, &current_path, &mut new_action, &mut self.expanded, 0.0, &mut needs_refresh, media_slot);
    ...
    fn handle_action_dialog(&mut self, ctx: &egui::Context, ed: &mut Editor, media_slot: &mut Option<PathBuf>) {
                ...
                FileAction::Rename { target, mut input } => {
                    egui::Window::new("✏️  Rename")
                        ...
                        .show(ctx, |ui| {
                            ...
                            ui.add_space(6.0);
                            let resp = ui.add(egui::TextEdit::singleline(&mut input).desired_width(360.0));
                            resp.request_focus();
...
fn show_nodes(
    ...
    for node in nodes {
        if node.is_dir {
            ...
            resp.dnd_set_drag_payload(DragItem { path: node.path.clone() });
            if resp.clicked() {
            ...
            if open {
                show_nodes(ui, &node.children, ed, p, current_path, pending_action, expanded, indent + INDENT, needs_refresh, media_slot);
            }
        } else {
...
fn dir_row(ui: &mut egui::Ui, name: &str, open: bool, indent: f32, p: &Palette) -> egui::Response {
    ...
    if is_drop {
        ui.painter().rect_stroke(
            rect.shrink(1.0), Rounding::same(5.0),
    ...
    ui.painter().text(
        pos2(left, cy), Align2::LEFT_CENTER,
        if open { "▾" } else { "▸" },
        FontId::proportional(12.0), p.text_dim,
    );
    ui.painter().text(
        pos2(left + 14.0, cy), Align2::LEFT_CENTER,
        "📁", FontId::proportional(13.0), p.text_dim,
    );
    ui.painter().text(
        pos2(left + 34.0, cy), Align2::LEFT_CENTER,
        name, FontId::proportional(13.0), p.fg,
    );
    resp
}

fn file_row(ui: &mut egui::Ui, icon: &str, name: &str, indent: f32, is_active: bool, p: &Palette) -> egui::Response {
    let sense = Sense::click_and_drag();
    let (rect, resp) = ui.allocate_exact_size(Vec2::new(ui.available_width(), ROW_H), sense);
    let bg = if is_active { p.selection } else if resp.hovered() { p.faint_bg } else { Color32::TRANSPARENT };
    if bg != Color32::TRANSPARENT {
        ui.painter().rect_filled(rect, Rounding::same(5.0), bg);
    }
    ...
    ui.painter().text(
        pos2(left, cy), Align2::LEFT_CENTER,
        icon, FontId::proportional(13.0),
        if is_active { p.accent } else { p.text_dim },
    );
    ui.painter().text(
        pos2(left + 22.0, cy), Align2::LEFT_CENTER,
        name, FontId::proportional(13.0), p.fg,
    );
    resp
}
...
fn perform_move(ed: &mut Editor, drag: &DragItem, target_dir: &Path) -> bool {
    ...
    let Some(name) = drag.path.file_name() else { return false };
...
fn icon_btn(ui: &mut egui::Ui, glyph: &str, tip: &str, p: &Palette) -> egui::Response {
    let resp = ui.add(egui::Button::new(RichText::new(glyph).size(12.0).color(p.text_dim))
        .frame(false)
        .min_size(Vec2::new(22.0, 20.0)));
    if resp.hovered() {
        ui.painter().rect_filled(resp.rect, Rounding::same(4.0), p.faint_bg);
    }
