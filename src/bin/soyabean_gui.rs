...
impl GuiApp {
    ...
    /// Open a file, routing images/audio to the media viewer and everything
    /// else to the text editor.
    fn open_path(&mut self, path: PathBuf) {
        if media_view::is_media_path(&path) {
            self.media_path = Some(path);
        } else {
            self.media_path = None;
            self.ed.open_file(path);
        }
    }
    ...
    fn handle_events(
        ...
        let term_body_focused =
            self.show_terminal && self.terminal.as_ref().map_or(false, |t| t.focused);
        ...
        for e in events {
            ...
            match e {
                egui::Event::Key {
                    ...
                    let ctrl = modifiers.ctrl || modifiers.command;
                    if ctrl && *key == egui::Key::C && !has_copy {
                        self.do_copy(ctx, false);
                        continue;
                    }
                    if ctrl && *key == egui::Key::X && !has_cut {
                    ...
                    if let Some(ck) = to_ckey(*key, *modifiers) {
                        let printable =
                            matches!(ck.code, KeyCode::Char(ch) if is_printable_char(ch));
    ...
    fn command_list(&self) -> Vec<(String, Cmd)> {
        ...
        vec![
            (
                "Open File… (Ctrl+P)".to_string(),
                Cmd::Key(KeyCode::Char('p'), ctrl),
            ),
            (
                "New File (Ctrl+N)".to_string(),
                Cmd::Key(KeyCode::Char('n'), ctrl),
            ),
            ...
            (
                ...
                Cmd::Key(KeyCode::Char('s'), ctrl),
            ),
            ("Save As… (Ctrl+Shift+S)".to_string(), Cmd::SaveAs),
            (
                "Close Buffer (Ctrl+W)".to_string(),
                Cmd::Key(KeyCode::Char('w'), ctrl),
            ),
            (
                "Toggle Integrated Terminal (Ctrl+`)".to_string(),
                Cmd::ToggleTerminal,
            ),
            ("Open OS Terminal Window".to_string(), Cmd::ExternalTerminal),
            (
                "Find… (Ctrl+F)".to_string(),
                Cmd::Key(KeyCode::Char('f'), ctrl),
            ),
            (
                "Find & Replace… (Ctrl+H)".to_string(),
                Cmd::Key(KeyCode::Char('h'), ctrl),
            ),
            (
                "Go to Line… (Ctrl+G)".to_string(),
                Cmd::Key(KeyCode::Char('g'), ctrl),
            ),
            (
                "Toggle Line Comment (Ctrl+/)".to_string(),
                Cmd::Key(KeyCode::Char('/'), ctrl),
            ),
            (
                "Duplicate Line (Ctrl+D)".to_string(),
                Cmd::Key(KeyCode::Char('d'), ctrl),
            ),
            (
                "Delete Line (Ctrl+K)".to_string(),
                Cmd::Key(KeyCode::Char('k'), ctrl),
            ),
            (
                "Move Line Up (Alt+↑)".to_string(),
                Cmd::Key(KeyCode::Up, alt),
            ),
            (
                "Move Line Down (Alt+↓)".to_string(),
                Cmd::Key(KeyCode::Down, alt),
            ),
            (
                "Undo (Ctrl+Z)".to_string(),
                Cmd::Key(KeyCode::Char('z'), ctrl),
            ),
            (
                "Redo (Ctrl+Y)".to_string(),
                Cmd::Key(KeyCode::Char('y'), ctrl),
            ),
            (
                "Next Buffer (Alt+→)".to_string(),
                Cmd::Key(KeyCode::Right, alt),
            ),
            (
                "Previous Buffer (Alt+←)".to_string(),
                Cmd::Key(KeyCode::Left, alt),
            ),
            ("Zoom In (Ctrl+=)".to_string(), Cmd::ZoomIn),
            ("Zoom Out (Ctrl+-)".to_string(), Cmd::ZoomOut),
            ("Reset Zoom (Ctrl+0)".to_string(), Cmd::ResetZoom),
            (
                "Switch Theme (Dark / Light / Tokyo)".to_string(),
                Cmd::ToggleTheme,
            ),
    ...
    fn palette_matches(&self) -> Vec<(String, Cmd)> {
        let mut scored: Vec<(i64, String, Cmd)> = self
            .command_list()
            .into_iter()
            .filter_map(|(label, c)| {
                let l = label.clone();
    ...
    fn tab_bar(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            for (i, name, dirty) in &tabs {
                ...
                let text = format!("{} {}{}", icon, if *dirty { "● " } else { "" }, name);
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
                            if ui
                                .selectable_label(active, RichText::new(&text).color(fg).strong())
                                .clicked()
                            {
                                click = Some(*i);
                            }
                            if ui
                                .small_button(RichText::new("×").color(p.text_dim))
                                .clicked()
                            {
                                close = Some(*i);
                            }
    ...
    fn close_buffer(&mut self, i: usize) {
        let dirty = self
            ...
            .bufs
            .get(i)
            .is_some_and(|b| b.dirty && !b.is_scratch);
        if dirty && self.confirm_close != Some(i) {
            self.confirm_close = Some(i);
            self.ed
                .msg("Unsaved changes — click close again to discard");
            return;
        }
        self.confirm_close = None;
        self.ed.bufs.remove(i);
        if self.ed.bufs.is_empty() {
            self.ed.bufs.push(soyabean::buffer::Buffer::empty());
        }
        if self.ed.cur >= self.ed.bufs.len() {
            self.ed.cur = self.ed.bufs.len() - 1;
        }
    }
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.colored_label(
                        ...
                        format!(
                            "{} / {} files",
                            self.ed.finder.matched.len(),
                            self.ed.finder.files.len()
                        ),
                    );
                });
                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        for (i, (label, cmd)) in matches.iter().enumerate() {
                            ...
                            let fg = if active { p.accent } else { p.fg };
                            if ui
                                .selectable_label(active, RichText::new(label).color(fg))
                                .clicked()
                            {
                                run = Some(*cmd);
                            }
                ...
                ui.vertical_centered(|ui| {
                    ...
                    ui.label(
                        RichText::new("soyabean")
                            .size(32.0)
                            .strong()
                            .color(p.welcome_title),
                    );
                    ui.label(
                        RichText::new("Next-Gen Fast & Lightweight Code IDE")
                            .size(14.0)
                            .color(p.welcome_sub),
                    );
                    ui.add_space(4.0);
                    ui.label(
                        RichText::new("crafted by zenx")
                            .size(12.0)
                            .italics()
                            .color(p.text_dim),
                    );
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .add(
                                egui::Button::new(RichText::new("🌙 Dark").size(13.0))
                        ...
                        if ui
                            .add(
                                egui::Button::new(RichText::new("☀ Light").size(13.0))
                                    .fill(if self.theme == Theme::Light {
                                        p.accent
                                    } else {
                                        p.button_bg
                                    })
                        ...
                        if ui
                            .add(
                                egui::Button::new(RichText::new("🌌 Tokyo").size(13.0))
                                    .fill(if self.theme == Theme::TokyoNight {
                                        p.accent
                                    } else {
                                        p.button_bg
                                    })
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .add(
                                egui::Button::new(RichText::new("📂 Open File").size(13.0))
                        ...
                        if ui
                            .add(
                                egui::Button::new(RichText::new("📄 New File").size(13.0))
                                    .min_size(Vec2::new(150.0, 34.0)),
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .add(
                                egui::Button::new(RichText::new("🖥 Terminal").size(13.0))
                                    .min_size(Vec2::new(150.0, 34.0)),
                            )
                            .clicked()
                        {
                    ...
                    if !recents.is_empty() {
                        ...
                        for path in &recents {
                            let name = path
                                ...
                                .unwrap_or_default();
                            if ui
                                .link(RichText::new(format!("  📄 {}", name)).color(p.accent))
                                .clicked()
                            {
        ...
        egui::Window::new("About soyabean")
            ...
            .show(ctx, |ui| {
                ui.vertical_centered(|ui| {
                    ...
                    ui.label(
                        RichText::new(
                            "A fast, modern code IDE with integrated terminal & folder tools",
                        )
                        .size(13.0)
                        .color(p.fg),
                    );
                    ...
                    ui.add_space(8.0);
                    ui.label(
                        RichText::new("crafted by  zenx ヾ(≧▽≦*)o")
                            .size(14.0)
                            .strong()
                            .color(p.accent),
                    );
                    ui.add_space(4.0);
                    ui.label(
                        RichText::new("Built with Rust + egui + portable-pty")
                            .size(11.0)
                            .color(p.text_dim),
                    );
                    ui.add_space(12.0);
...
impl eframe::App for GuiApp {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::SidePanel::left("tree")
            ...
            .show(ctx, |ui| {
                self.tree.show(ui, &mut self.ed, p, &mut self.media_path);
            });
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| match &self.media_path {
                Some(mp) if media_view::is_image_path(mp) => {
                    media_view::show_image(ui, mp, p, &mut self.img_cache);
                }
                Some(mp) if media_view::is_audio_path(mp) => {
                    media_view::show_audio(ui, mp, p, &mut self.audio_player);
                }
                _ => {
...
fn icon_btn<'a>(ui: &mut egui::Ui, icon: &str, tip: &str, p: &Palette) -> egui::Response {
    ui.add(
        egui::Button::new(RichText::new(icon).size(14.0).color(p.fg))
            .fill(p.button_bg)
...
fn to_ckey(key: egui::Key, mods: egui::Modifiers) -> Option<KeyEvent> {
    ...
    let code = match key {
        ...
        K::Y => KeyCode::Char('y'),
        K::Z => KeyCode::Char('z'),
        K::Num0 => KeyCode::Char('0'),
        K::Num1 => KeyCode::Char('1'),
        K::Num2 => KeyCode::Char('2'),
        K::Num3 => KeyCode::Char('3'),
        K::Num4 => KeyCode::Char('4'),
        K::Num5 => KeyCode::Char('5'),
        K::Num6 => KeyCode::Char('6'),
        K::Num7 => KeyCode::Char('7'),
        K::Num8 => KeyCode::Char('8'),
        K::Num9 => KeyCode::Char('9'),
...
/// Install nicer system fonts: Segoe UI for the UI, Cascadia/Consolas for code.
/// Falls back silently to egui's bundled fonts when none are found.
fn install_fonts(ctx: &egui::Context) {
    ...
    #[cfg(target_os = "windows")]
    {
        let font_dir = std::env::var("SystemRoot")
            .map(|r| format!("{}\\Fonts", r))
            .unwrap_or_else(|_| "C:\\Windows\\Fonts".to_string());
        let load = |name: &str| std::fs::read(format!("{}\\{}", font_dir, name)).ok();

        if let Some(data) = load("segoeui.ttf") {
            fonts
                .font_data
                .insert("segoe".to_owned(), egui::FontData::from_owned(data));
            fonts
                .families
        ...
        if let Some(data) = load("segoeuib.ttf") {
            fonts
                .font_data
                .insert("segoe-bold".to_owned(), egui::FontData::from_owned(data));
            fonts
                .families
                .get_mut(&egui::FontFamily::Proportional)
                .unwrap()
                .push("segoe-bold".to_owned());
        }

        for name in ["CascadiaCode.ttf", "CascadiaMono.ttf", "consola.ttf"] {
            if let Some(data) = load(name) {
                fonts
                    .font_data
                    .insert("mono".to_owned(), egui::FontData::from_owned(data));
                fonts
                    .families
